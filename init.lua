...
local CONFIG = {
  ...
  findTimeoutSeat = 0.7,
  findTimeoutNotes = 2.5,
  refreshStopTimeout = 4.0,
  confirmTimeout = 3.0,
  confirmPoll = 0.10,
...
local function safeControllerCall(fn, ...)
  local ok, status, body = pcall(fn, ...)
  if ok then
    return status, body
  end
  local err = tostring(status)
  log.e("Controller callback error: " .. err)
  return 500, err
end
...
local function findConfiguredCanvasPair(screens)
  ...
  for _, screen in ipairs(screens) do
    ...
    local matchesConfiguredCanvas = screenNameMatches(screen, CONFIG.canvasDisplayNameHints)
    if matchesConfiguredCanvas then
...
isSlidePanelFrame = function(frame)
  return frame
...
local function resolveSlideScreens(side, screens, halfCanvasCandidates, fullCanvasCandidates)
  ...
  if configuredLeft or configuredRight then
    ...
    if chosenSlide then
      if configuredLeft and configuredRight then
        local configuredFrame = select(1, resolveStitchedFrame({ configuredLeft, configuredRight }))
        if configuredFrame then
          log.i(string.format(
            "Using configured canvas pair for side=%s with full 5760x2160 span -> slide=%s alternate=%s",
            side,
            safeScreenFingerprint(chosenSlide),
            safeScreenFingerprint(chosenAlternate)
          ))
          return chosenSlide, chosenAlternate, configuredFrame
        end
      end
      log.i(string.format(
  ...
  if #fullCanvasCandidates >= 1 then
    ...
    if #halfCanvasCandidates >= 2 then
      ...
      if stitchedFrame then
        log.i("Using stitched 5760x2160 span for slideshow")
        return useLeftSide and stitchedLeft or stitchedRight, useLeftSide and stitchedRight or stitchedLeft, stitchedFrame
      end
    end
  ...
  if stitchedFrame then
    log.i("Detected stitched 2880x2160 output pair; using full 5760x2160 span")
    return useLeftSide and stitchedLeft or stitchedRight, useLeftSide and stitchedRight or stitchedLeft, stitchedFrame
  end
...
local function isKnownKeynoteBundleId(bundleId)
  for _, candidate in ipairs(CONFIG.keynoteBundleIds or {}) do
    if bundleId == candidate then
      return true
    end
  end
  return false
...
local function keynoteWindowCount(app)
  if not app then
    return 0
  end
...
local function keynoteBundleRank(bundleId)
  for index, candidate in ipairs(CONFIG.keynoteBundleIds or {}) do
    if bundleId == candidate then
      return index
    end
  end
...
local function findKeynoteApp()
  ...
  for _, bundleId in ipairs(CONFIG.keynoteBundleIds or {}) do
    local app = hs.application.get(bundleId)
    if app and not seen[app:pid()] then
      apps[#apps + 1] = app
      seen[app:pid()] = true
    end
  end
  ...
  for _, app in ipairs(hs.application.runningApplications() or {}) do
    if isKnownKeynoteBundleId(app:bundleID()) and not seen[app:pid()] then
      apps[#apps + 1] = app
      seen[app:pid()] = true
    end
  end
  ...
  table.sort(apps, function(a, b)
    local aWindows = keynoteWindowCount(a)
    local bWindows = keynoteWindowCount(b)
    if aWindows ~= bWindows then
      return aWindows > bWindows
    end
    local aRank = keynoteBundleRank(a:bundleID())
    local bRank = keynoteBundleRank(b:bundleID())
    if aRank ~= bRank then
      return aRank < bRank
    end
    if a:isFrontmost() ~= b:isFrontmost() then
      return a:isFrontmost()
    end
    return a:pid() < b:pid()
  end)
  ...
  return hs.application.find(CONFIG.keynoteName)
end
...
local function launchAndActivateKeynote()
  ...
  if app then
    app:activate(true)
  elseif hs.application.launchOrFocusByBundleID then
  ...
  while hs.timer.secondsSinceEpoch() < deadline do
    local activeApp = findKeynoteApp()
    if activeApp then
      activeApp:activate(true)
...
local function isKeynoteWindow(win)
  ...
  return app and isKnownKeynoteBundleId(app:bundleID())
end

local function runAppleScript(script)
  local ok, result, descriptor = hs.osascript.applescript(script)
  if ok then
    return true, result, descriptor
  end
  return false, result, descriptor
end

local function startDocumentWithAppleScript(bundleId)
  if not bundleId or bundleId == "" then
    return false, "Missing Keynote bundle ID"
  end

  local script = string.format([[
tell application id %q
  if (count of documents) = 0 then error "No open Keynote document"
  start (front document)
end tell
]], bundleId)

  local ok, result, descriptor = runAppleScript(script)
  if ok then
    log.i("Started playback via AppleScript for " .. bundleId)
    return true, nil
  end

  local errText = string.format("%s %s", tostring(result or ""), tostring(descriptor or ""))
  if errText:find("already playing", 1, true) then
    log.i("Keynote document already playing for " .. bundleId)
    return true, nil
  end

  log.w("AppleScript start failed for " .. bundleId .. ": " .. errText)
  return false, errText
end

local function showPresenterDisplayInWindow(bundleId)
  if not bundleId or bundleId == "" then
    return false, "Missing Keynote bundle ID"
  end
...
tell application "System Events"
  tell (first application process whose bundle identifier is %q)
    click menu item "Show Presenter Display in Window" of menu 1 of menu bar item "Play" of menu bar 1
  end tell
end tell
]], bundleId)
  ...
  local ok, result, descriptor = runAppleScript(script)
  if ok then
    log.i("Enabled Presenter Display in Window for " .. bundleId)
    return true, nil
  end

  local errText = string.format("%s %s", tostring(result or ""), tostring(descriptor or ""))
  log.w("Presenter Display in Window toggle failed for " .. bundleId .. ": " .. errText)
  return false, errText
end

local function keynoteIsPlaying(bundleId)
  if not bundleId or bundleId == "" then
    return false
  end
...
tell application id %q
  ...
  local ok, result = runAppleScript(script)
  if not ok then
    return false
  end
  return result == true
...
local function keynoteSlideCount(bundleId)
  if not bundleId or bundleId == "" then
    return nil, "Missing Keynote bundle ID"
  end
...
tell application id %q
  ...
  return nil, string.format("%s %s", tostring(result or ""), tostring(descriptor or ""))
end

local function keynoteCurrentSlideMatches(bundleId, slideNumber)
  if not bundleId or bundleId == "" then
    return false, "Missing Keynote bundle ID"
  end
...
tell application id %q
  if (count of documents) = 0 then error "No open Keynote document"
  return (current slide of front document is slide %d of front document) as string
end tell
]], bundleId, slideNumber)

  local ok, result, descriptor = runAppleScript(script)
  if ok then
    return tostring(result) == "true", nil
  end
  return false, string.format("%s %s", tostring(result or ""), tostring(descriptor or ""))
end

local function showSlideWithAppleScript(bundleId, slideNumber)
  if not bundleId or bundleId == "" then
    return false, "Missing Keynote bundle ID"
  end
...
tell application id %q
  if (count of documents) = 0 then error "No open Keynote document"
  if not playing then error "Slideshow is not active"
  show slide %d of front document
end tell
]], bundleId, slideNumber)
...
local function gotoSlide(slideNumber)
  local app, appErr = getRunningKeynote()
  if not app then
    return 404, appErr
  end

  local bundleId = app:bundleID()
  if not keynoteIsPlaying(bundleId) then
    return 409, "Slideshow is not active (run /keynote/run first)"
  end

  local slideCount, countErr = keynoteSlideCount(bundleId)
  if not slideCount then
    return 500, countErr or "Could not determine slide count"
  end
  if slideNumber > slideCount then
    return 400, string.format("Invalid slide %d (deck has %d slides)", slideNumber, slideCount)
  end

  local shown, showErr = showSlideWithAppleScript(bundleId, slideNumber)
  if not shown then
    return 500, showErr or "Could not change slides"
  end

  local deadline = hs.timer.secondsSinceEpoch() + CONFIG.confirmTimeout
  local lastErr = "Slide change could not be confirmed"
  while hs.timer.secondsSinceEpoch() < deadline do
    local matches, matchErr = keynoteCurrentSlideMatches(bundleId, slideNumber)
    if matches then
      return 200, "OK"
    end
    if matchErr and matchErr ~= "" then
      lastErr = matchErr
    end
    hs.timer.usleep(ms(CONFIG.confirmPoll))
  end

  return 500, lastErr
end

local function startSlideshowPlayback(app)
  local bundleId = app and app:bundleID() or nil
  local ok = startDocumentWithAppleScript(bundleId)
  if not ok then
    hs.eventtap.keyStroke(CONFIG.playShortcut.mods, CONFIG.playShortcut.key, 0, app)
    log.i("Fell back to Option+Command+P to start Play in Window")
    return true
  end

  hs.timer.usleep(ms(CONFIG.playDelay))
  showPresenterDisplayInWindow(bundleId)
  return true
end
...
local function safeWindowFrame(win)
  if not win then
    return nil
  end
...
local function windowMatchesTarget(win, target, tolerance)
  local frame = safeWindowFrame(win)
  if not frame or not target then
    return false, frame
  end
  return framesClose(frame, target, tolerance or CONFIG.frameTolerance), frame
end

local function confirmPresentationState(app, slideWindow, slideTarget, notesWindow, notesTarget, requirePlaying)
  local bundleId = app and app:bundleID() or nil
  local deadline = hs.timer.secondsSinceEpoch() + CONFIG.confirmTimeout
  local lastReason = "confirmation timed out"

  while hs.timer.secondsSinceEpoch() < deadline do
    local playingOk = true
    if requirePlaying then
      playingOk = keynoteIsPlaying(bundleId)
      if not playingOk then
        lastReason = "Keynote playback not active yet"
      end
    end

    local slideOk, slideFrame = windowMatchesTarget(slideWindow, slideTarget, CONFIG.frameTolerance * 6)
    if not slideOk then
      lastReason = "Slide window not settled on target"
      if slideFrame then
        lastReason = lastReason .. " (" .. frameToString(slideFrame) .. ")"
      end
    end

    local notesOk = true
    if notesWindow and notesTarget then
      notesOk = select(1, windowMatchesTarget(notesWindow, notesTarget, CONFIG.fullFrameTolerance * 2))
      if not notesOk then
        local isFullscreen = false
        if notesWindow.isFullScreen then
          local ok, result = pcall(function()
            return notesWindow:isFullScreen()
          end)
          isFullscreen = ok and result == true
        end
        if isFullscreen then
          notesOk = true
        else
          local notesFrame = safeWindowFrame(notesWindow)
          lastReason = "Presenter window not settled on target"
          if notesFrame then
            lastReason = lastReason .. " (" .. frameToString(notesFrame) .. ")"
          end
        end
      end
    end

    if playingOk and slideOk and notesOk then
      return true, nil
    end
...
local function startSlideshowAndSeat(side, mode)
  ...
  local refreshBeforeStart = (mode == "refresh")
  ...
  if refreshBeforeStart then
    local refreshed, refreshErr = refreshPlaybackIfNeeded(app)
    if not refreshed then
      return 500, refreshErr or "Could not refresh active slideshow"
    end
    local relaunchedApp, relaunchedErr = launchAndActivateKeynote()
    if not relaunchedApp then
      return 404, relaunchedErr
    end
    app = relaunchedApp
  end
  ...
  local confirmed, confirmErr = confirmPresentationState(
    app,
    slideWindow,
    targetFrame,
    notesWindow,
  ...
  if not confirmed then
    return 500, confirmErr or "Presentation state could not be confirmed"
  end
...
refreshPlaybackIfNeeded = function(app)
  if not app then
    return true, nil
  end
  ...
  log.i("Refresh requested while playing; stopping playback before restart")
  local status, body = stopSlideshow()
  if status ~= 200 then
    return false, body
  end

  local deadline = hs.timer.secondsSinceEpoch() + CONFIG.refreshStopTimeout
  while hs.timer.secondsSinceEpoch() < deadline do
    if not keynoteIsPlaying(bundleId) then
      break
    end
  ...
  log.i(string.format("Waiting %.2fs for deck refresh before restart", CONFIG.refreshDelay))
  hs.timer.usleep(ms(CONFIG.refreshDelay))
  return true, nil
end
...
local function handleRequest(method, rawPath, _headers, _body)
  ...
  if path == "/keynote/left" then
    local status, body = safeControllerCall(startSlideshowAndSeat, "left", "start")
    return plainText(status, body)
  end

  if path == "/keynote/run" then
    local side = normalizeSide(CONFIG.hotkeySide) or "left"
    local status, body = safeControllerCall(startSlideshowAndSeat, side, "refresh")
    return plainText(status, body)
  end

  if path == "/keynote/goto" then
    local slideNumber = parsePositiveInteger(query.slide or query.number or query.n)
    if not slideNumber then
      return plainText(400, "Invalid slide (use ?slide=25)")
    end
    local status, body = safeControllerCall(gotoSlide, slideNumber)
    return plainText(status, body)
  end

  if path == "/keynote/right" then
    local status, body = safeControllerCall(startSlideshowAndSeat, "right", "start")
    return plainText(status, body)
  end
  ...
  if path == "/keynote/seat" then
    ...
    local status, body = safeControllerCall(startSlideshowAndSeat, side, "seat")
    return plainText(status, body)
  end

  if path == "/keynote/stop" then
    local status, body = safeControllerCall(stopSlideshow)
    return plainText(status, body)
  end
...
local assignedHotkey = hs.hotkey.systemAssigned({ "cmd", "alt", "ctrl" }, "k")
if assignedHotkey then
  log.w("Hotkey CMD+ALT+CTRL+K is already assigned by the system")
end

if _G.keynoteHotkey then
  _G.keynoteHotkey:delete()
  _G.keynoteHotkey = nil
end

_G.keynoteHotkey = hs.hotkey.bind({ "cmd", "alt", "ctrl" }, "k", function()
  log.i("Hotkey pressed: CMD+ALT+CTRL+K")
  local ok, status, body = pcall(startSlideshowAndSeat, CONFIG.hotkeySide, "refresh")
  if not ok then
    local err = tostring(status)
...
if _G.keynoteHotkey and _G.keynoteHotkey.enable then
  _G.keynoteHotkey:enable()
end
