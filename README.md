...
- Starts an HTTP server on port `8765` with:
  - `GET /keynote/run` → launch/activate Keynote using the default start side and seat slideshow
  - `GET /keynote/goto?slide=25` → jump the active slideshow to a specific slide number
...
curl http://MAC_IP:8765/keynote/run
curl "http://MAC_IP:8765/keynote/goto?slide=25"
...
- `http://MAC_IP:8765/keynote/run`
- `http://MAC_IP:8765/keynote/goto?slide=25`
